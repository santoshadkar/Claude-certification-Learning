...
export const questions: QuestionInput[] = [
  ...
  },
  {
    ...
    options: [
      { id: "A", text: "Both are infrastructure requirements with no functional component." },
      { id: "B", text: "The upload size limit is largely a functional/infrastructure constraint on input handling, while the response-time target is a latency requirement that should inform model tier and API pattern choice." },
      { id: "C", text: "Both requirements are purely cosmetic and don't affect implementation." },
      { id: "D", text: "Requirements like these never affect architecture decisions." },
    ],
    correctOptionIds: ["B"],
    selectCount: 1,
    explanation:
      "Distinguishing what kind of constraint each requirement represents — input handling capacity versus a latency target — is what lets a developer translate it into concrete design decisions like model tier and API pattern. Treating both as purely infrastructure (A) or cosmetic (C) misses this, and requirements like these routinely do shape architecture (D wrong).",
    citation: "Claude Certified Developer – Foundations Exam Guide §6, Domain 2 — 'Understanding Requirements'",
    difficulty: "medium",
  },
  {
    id: "dev-ai-008",
    ...
    options: [
      ...
      { id: "B", text: "Choose a model tier and API pattern (real-time vs. batch) based on the clarified, concrete target once it exists." },
      ...
      { id: "D", text: "Assume 'feel fast' has no bearing on any technical decision and can be ignored entirely." },
    ],
    correctOptionIds: ["A", "B"],
    selectCount: 2,
    explanation:
      "Translating vague language into a measurable target (A) and then using that target to inform concrete choices like model tier and API pattern (B) is the correct sequence. Proceeding without clarification (C) risks building the wrong thing, and ignoring the requirement (D) discards a real signal about what's needed.",
    citation: "Claude Certified Developer – Foundations Exam Guide §6, Domain 2 — 'Understanding Requirements'",
    difficulty: "medium",
  },
  {
    ...
    options: [
      { id: "A", text: "Only manually, at the developer's discretion, with no fixed place in the pipeline." },
      ...
      { id: "C", text: "Only after a production incident has already occurred." },
      { id: "D", text: "Evals have no natural place in an SDLC and should be run independently of any pipeline." },
    ],
    correctOptionIds: ["B"],
    selectCount: 1,
    explanation:
      "Running evals automatically in CI, akin to unit tests, catches regressions from prompt or model changes before they merge — leaving it purely to discretion (A) or only after an incident (C) both miss regressions earlier detection would catch, and evals do have a natural, integrable place in the pipeline (D wrong).",
    citation: "Claude Certified Developer – Foundations Exam Guide §6, Domain 2 — 'Systems Life Cycle'",
    difficulty: "medium",
  },
  {
    id: "dev-ai-010",
    ...
    options: [
      { id: "A", text: "Streaming — displaying tokens incrementally as they're generated, rather than waiting for the complete response." },
      { id: "B", text: "Switching to the Message Batches API for this live interaction." },
      { id: "C", text: "Increasing max_tokens to allow a longer response." },
      { id: "D", text: "Disabling the system prompt entirely." },
    ],
    correctOptionIds: ["A"],
    selectCount: 1,
    explanation:
      "Streaming directly addresses perceived latency by showing output incrementally rather than all at once. The Batches API (B) is for asynchronous, non-live workloads and would make this worse, increasing max_tokens (C) doesn't address responsiveness, and removing the system prompt (D) is unrelated to the perceived-speed problem.",
    citation: "Claude Certified Developer – Foundations Exam Guide §6, Domain 2 — 'Claude API Mechanics'",
    difficulty: "easy",
  },
  {
    id: "dev-ai-011",
    ...
    options: [
      ...
      { id: "B", text: "Extended thinking, which is unrelated to accepting image input." },
      { id: "C", text: "The Message Batches API, which is required for any image-based request." },
      { id: "D", text: "Prompt caching, which has no bearing on accepting image input." },
    ],
    correctOptionIds: ["A"],
    selectCount: 1,
    explanation:
      "Vision input is exactly what's needed to let Claude analyze image content directly. Extended thinking (B) and prompt caching (D) are unrelated capabilities, and the batch API (C) is about processing pattern, not a requirement for handling images.",
    citation: "Claude Certified Developer – Foundations Exam Guide §6, Domain 2 — 'Claude API Mechanics'",
    difficulty: "easy",
  },
  {
    ...
    options: [
      ...
      { id: "B", text: "Extended thinking adds latency and token cost that likely isn't justified for a simple, well-defined task that doesn't need deeper reasoning." },
      { id: "C", text: "Extended thinking only affects image inputs, not text formatting tasks." },
      { id: "D", text: "Extended thinking has no effect on latency or cost." },
    ],
    correctOptionIds: ["B"],
    selectCount: 1,
    explanation:
      "Extended thinking trades latency and token cost for deeper reasoning — applying it to a simple task that doesn't need that depth wastes that cost for no benefit, contrary to A's blanket recommendation. It applies broadly, not just to images (C wrong), and it does have real cost/latency impact (D wrong).",
    citation: "Claude Certified Developer – Foundations Exam Guide §6, Domain 2 — 'Claude API Mechanics'",
    difficulty: "medium",
  },
  {
    ...
    options: [
      { id: "A", text: "Place the varying user question first, followed by the static system prompt and reference document." },
      ...
      { id: "C", text: "Order doesn't matter for prompt caching to be effective." },
      { id: "D", text: "Prompt caching only works for the user's question, never for a system prompt." },
    ],
    correctOptionIds: ["B"],
    selectCount: 1,
    explanation:
      "Prompt caching works by reusing a stable prefix, so the static content needs to come first, with variable content afterward — reversing the order (A) breaks the reusable-prefix structure caching depends on, order does matter (C wrong), and system prompts and reference material are exactly the kind of content caching is meant to reuse (D wrong).",
    citation: "Claude Certified Developer – Foundations Exam Guide §6, Domain 2 — 'Claude API Mechanics'",
    difficulty: "medium",
  },
  {
    ...
    options: [
      { id: "A", text: "The core request/response mechanics (messages, tools, streaming) generally stay conceptually similar, while model identifiers and authentication typically differ by platform." },
      { id: "B", text: "Nothing at all differs between direct API access and third-party platform access." },
      { id: "C", text: "Tool use is only available through Anthropic's direct API, never through third-party platforms." },
      { id: "D", text: "Streaming is never available when accessing Claude through a third-party platform." },
    ],
    correctOptionIds: ["A"],
    selectCount: 1,
    explanation:
      "The underlying Messages API mechanics stay conceptually consistent across access paths, while platform-specific details like model IDs and auth do differ — some difference does exist (B wrong), and core capabilities like tool use (C) and streaming (D) aren't exclusive to the direct API.",
    citation: "Claude Certified Developer – Foundations Exam Guide §6, Domain 2 — 'Claude API Mechanics'",
    difficulty: "medium",
  },
  {
    ...
    options: [
      ...
      { id: "C", text: "Model selection, since a more capable model would prevent the timeout." },
      { id: "D", text: "This has no relevant software engineering principle and must be handled manually after the fact each time." },
    ],
    correctOptionIds: ["A"],
    selectCount: 1,
    explanation:
      "Idempotency is the standard software engineering concept for exactly this problem — safely retrying an operation without duplicating its effect. This isn't a prompt-wording issue (B) or something model capability would fix (C), and it's a well-established pattern, not something requiring manual after-the-fact handling every time (D).",
    citation: "Claude Certified Developer – Foundations Exam Guide §6, Domain 2 — 'Software Engineering Foundations'",
    difficulty: "medium",
  },
  {
    id: "dev-ai-016",
    ...
    options: [
      { id: "A", text: "Input validation — checking that data conforms to its expected shape before using it, which is standard practice for any external or generated input, not just Claude's." },
      { id: "B", text: "Prompt caching, which is unrelated to input validation." },
      { id: "C", text: "Model tier selection, which has no bearing on this practice." },
      { id: "D", text: "Nothing is being skipped, since Claude always returns perfectly-shaped input matching the schema." },
    ],
    correctOptionIds: ["A"],
    selectCount: 1,
    explanation:
      "Validating structured input against its schema before use is standard defensive engineering practice, applicable to any external or model-generated input — this isn't related to caching (B) or model tier (C), and skipping validation is a real risk even with tool_use's structural guarantees, since semantic errors can still occur (D wrong).",
    citation: "Claude Certified Developer – Foundations Exam Guide §6, Domain 2 — 'Software Engineering Foundations'",
    difficulty: "medium",
  },
  {
    ...
    options: [
      { id: "A", text: "Version control — treating prompt changes like code changes, reviewed and tracked through the same process, so changes are recorded and reversible." },
      { id: "B", text: "Increasing the model's context window, which is unrelated to change tracking." },
      { id: "C", text: "Switching to a different model tier, which doesn't address the lack of change tracking." },
      { id: "D", text: "Nothing needs to change, since prompts don't need the same rigor as code." },
    ],
    correctOptionIds: ["A"],
    selectCount: 1,
    explanation:
      "Prompts that drive production behavior benefit from the same version control and review discipline as code — untracked, undocumented changes make it hard to know what changed or roll it back. Context window size (B) and model tier (C) are unrelated, and prompts absolutely warrant this rigor given their production impact (D wrong).",
    citation: "Claude Certified Developer – Foundations Exam Guide §6, Domain 2 — 'Software Engineering Foundations'",
    difficulty: "medium",
  },
  {
    ...
    options: [
      { id: "A", text: "It should skip code review entirely, since it was AI-assisted." },
      { id: "B", text: "It should go through the same code review process as any other change — correctness, style, and security review don't become optional just because AI assisted in writing it." },
      { id: "C", text: "It should receive a much lighter review than hand-written code, since AI-generated code is inherently more reliable." },
      { id: "D", text: "It should only be reviewed by the person who used Claude Code to generate it." },
    ],
    correctOptionIds: ["B"],
    selectCount: 1,
    explanation:
      "AI-assisted code needs the same review rigor as any other code change — how it was produced doesn't change whether it needs review (A, C both wrong), and self-review alone (D) doesn't provide the independent scrutiny normal code review is meant to provide.",
    citation: "Claude Certified Developer – Foundations Exam Guide §6, Domain 2 — 'Software Engineering Foundations'",
    difficulty: "easy",
  },
  {
    ...
    options: [
      ...
      { id: "C", text: "Add a sixth duplicate copy of the logic for a new feature to keep things consistent with the existing pattern." },
      { id: "D", text: "Delete all five implementations and stop building prompts programmatically altogether." },
    ],
    correctOptionIds: ["A"],
    selectCount: 1,
    explanation:
      "Extracting duplicated logic into a single shared function is standard refactoring practice that directly addresses the inconsistency problem — prompt-construction code deserves the same maintainability treatment as any other logic (B wrong), adding more duplication (C) makes the problem worse, and abandoning programmatic prompt construction entirely (D) is an overcorrection.",
    citation: "Claude Certified Developer – Foundations Exam Guide §6, Domain 2 — 'Software Engineering Foundations'",
    difficulty: "medium",
  },
  {
    ...
    options: [
      ...
      { id: "B", text: "Blurring the boundary between trusted instructions and untrusted retrieved content increases the risk that injected instructions inside the web content could be followed as if they were legitimate directives." },
      { id: "C", text: "The only risk is slightly higher token usage." },
      { id: "D", text: "This design pattern is always safe as long as the retrieved content is in English." },
    ],
    correctOptionIds: ["B"],
    selectCount: 1,
    explanation:
      "Failing to separate trusted instructions from untrusted content is exactly the design gap that enables prompt injection — this is a real content-boundary risk (A wrong), not merely a token-cost issue (C), and language of the content (D) is irrelevant to the underlying risk.",
    citation: "Claude Certified Developer – Foundations Exam Guide §6, Domain 2 — 'Claude Application Design'",
    difficulty: "medium",
  },
  {
    id: "dev-ai-021",
  ...
  },
  {
    ...
    options: [
      { id: "A", text: "Session hygiene — periodically trimming or summarizing resolved, irrelevant history so the active context stays focused on what's actually still relevant." },
      { id: "B", text: "Increasing max_tokens indefinitely to accommodate all history regardless of relevance." },
      { id: "C", text: "Disabling the system prompt once the session gets long." },
      { id: "D", text: "There is no design practice relevant to this situation." },
    ],
    correctOptionIds: ["A"],
    selectCount: 1,
    explanation:
      "Session hygiene — trimming or summarizing what's no longer relevant — directly addresses accumulated, irrelevant history degrading quality. Simply raising max_tokens (B) doesn't address relevance, disabling the system prompt (C) removes something unrelated and needed, and this is a well-recognized design concern, not a non-issue (D wrong).",
    citation: "Claude Certified Developer – Foundations Exam Guide §6, Domain 2 — 'Claude Application Design'",
    difficulty: "medium",
  },
  {
    ...
    options: [
      ...
      { id: "B", text: "Never allowing any plugin to be updated once created." },
      { id: "C", text: "Removing all plugins from every project to avoid the issue entirely." },
      { id: "D", text: "Plugin management has no bearing on this kind of breaking change." },
    ],
    correctOptionIds: ["A"],
    selectCount: 1,
    explanation:
      "Explicit versioning with deliberate opt-in upgrades is the standard way to prevent a shared dependency's breaking change from immediately propagating everywhere — freezing all plugins forever (B) or removing them entirely (C) both overcorrect, and plugin management practices are directly relevant here (D wrong).",
    citation: "Claude Certified Developer – Foundations Exam Guide §6, Domain 2 — 'Claude Application Design'",
    difficulty: "medium",
  },
  {
    ...
    options: [
      { id: "A", text: "No issue; a single shared system prompt is always the correct design regardless of surface or audience." },
      { id: "B", text: "The two surfaces have very different trust levels and audiences, so a system prompt written for a trusted internal engineer may not include appropriate boundaries for arbitrary public users." },
      { id: "C", text: "The only issue is that the prompt might be slightly too long for one of the two surfaces." },
      { id: "D", text: "System prompts have no effect on behavior differences across surfaces." },
    ],
    correctOptionIds: ["B"],
    selectCount: 1,
    explanation:
      "A prompt appropriate for a trusted internal tool may lack boundaries needed for an untrusted public-facing surface — reusing it unchanged across very different trust contexts is a real design gap (A wrong), it's not just a length concern (C), and system prompts are very much relevant to surface-specific behavior (D wrong).",
    citation: "Claude Certified Developer – Foundations Exam Guide §6, Domain 2 — 'Claude Application Design'",
    difficulty: "medium",
  },
  {
    ...
    options: [
      { id: "A", text: "Clearly delimit or structurally separate untrusted content from trusted instructions in the request." },
      { id: "B", text: "Apply guardrails or hooks so that instructions embedded within untrusted content can't trigger sensitive actions on their own." },
      { id: "C", text: "Concatenate all content — trusted and untrusted — into one undifferentiated block for simplicity." },
      { id: "D", text: "Trust that untrusted content will never contain adversarial instructions, since this is rare in practice." },
    ],
    correctOptionIds: ["A", "B"],
    selectCount: 2,
    explanation:
      "Structurally separating content by trust level (A) and enforcing guardrails against sensitive actions being triggered by untrusted content (B) are the recognized mitigations for this risk. Merging everything undifferentiated (C) recreates the risk, and assuming adversarial content is rare enough to ignore (D) is not a safe design assumption.",
    citation: "Claude Certified Developer – Foundations Exam Guide §6, Domain 2 — 'Claude Application Design'",
    difficulty: "medium",
  },
  {
    ...
    options: [
      { id: "A", text: "Prompt versioning — keeping track of distinct, identifiable versions of the prompt so a specific prior version can be restored quickly if needed." },
      { id: "B", text: "Always editing the live prompt directly with no history kept, since that's faster to deploy." },
      { id: "C", text: "Relying on memory to recall what the previous prompt said." },
      { id: "D", text: "Prompt versioning is not a meaningful practice for production systems." },
    ],
    correctOptionIds: ["A"],
    selectCount: 1,
    explanation:
      "Explicit prompt versioning is exactly what enables a fast, reliable rollback — editing live with no history (B) or relying on memory (C) both make rollback slow or unreliable, and this is a genuinely meaningful practice for any production system relying on a prompt (D wrong).",
    citation: "Claude Certified Developer – Foundations Exam Guide §6, Domain 2 — 'Configuration Management'",
    difficulty: "easy",
  },
  {
    id: "dev-ai-027",
    ...
    options: [
      { id: "A", text: "Pinning specific plugin versions in configuration, and upgrading them deliberately with testing, rather than letting updates apply automatically and silently." },
      { id: "B", text: "Removing all plugin dependencies permanently to avoid ever having this problem." },
      { id: "C", text: "Plugin dependency management has no bearing on production stability." },
      { id: "D", text: "Always using the newest available version of every plugin automatically, with no pinning at all." },
    ],
    correctOptionIds: ["A"],
    selectCount: 1,
    explanation:
      "Pinning dependency versions and upgrading deliberately with testing is the standard configuration-management practice that prevents silent breaking changes — removing all plugins (B) is an overcorrection, this is directly relevant to production stability (C wrong), and always auto-updating to the newest version (D) is exactly the practice that caused the problem.",
    citation: "Claude Certified Developer – Foundations Exam Guide §6, Domain 2 — 'Configuration Management'",
    difficulty: "medium",
  },
  {
    ...
    options: [
      ...
      { id: "B", text: "Both belong exclusively in CLAUDE.md, with settings.json serving no distinct purpose." },
      { id: "C", text: "Both belong exclusively in settings.json, with CLAUDE.md serving no distinct purpose." },
      { id: "D", text: "There is no meaningful distinction between these two configuration files." },
    ],
    correctOptionIds: ["A"],
    selectCount: 1,
    explanation:
      "CLAUDE.md and settings.json serve genuinely different configuration roles — behavioral/contextual guidance versus technical tool and permission configuration — so collapsing both into one file's purpose (B, C) or treating them as interchangeable (D) misrepresents how Claude Code's configuration is actually organized.",
    citation: "Claude Certified Developer – Foundations Exam Guide §6, Domain 2 — 'Configuration Management'",
    difficulty: "medium",
  },
];
